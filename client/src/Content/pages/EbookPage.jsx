...
export default function EbookPage() {
  ...
  const ebookOrders = CheckoutStateData.filter(
    (order) =>
      Array.isArray(order?.books) &&          // ← lowercase
      order.books.some((item) => item.format === "Ebook")
  );

  return (
    <>
      <HeroSection title="Ebook Orders" />
      <Order title="Ebook Orders" data={ebookOrders} />
    </>
  );
}