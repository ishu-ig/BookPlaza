...
import { getCategory, deleteCategory, updateCategory } from "../../Redux/ActionCreators/CategoryActionCreators";
...
export default function AdminCategory() {
  ...
  const totalCount    = CategoryStateData?.length ?? 0;
  const activeCount   = CategoryStateData?.filter((i) => i.active).length ?? 0;
  ...
  function updateRecord(_id) {
    const item = CategoryStateData.find((m) => m._id === _id);
    if (!item) return;
  ...
  useEffect(() => { dispatch(getCategory()); }, [flag]);

  const filteredData = CategoryStateData?.filter((item) =>
    ...
    <>
      ...
      <main className="dashboard-content">
        <div className="container-fluid px-3 px-lg-4 py-4">
          ...
          <section className="row g-3 mt-2 mb-1" aria-label="Category summary">
                        ...
                        <td className="text-end">
                          <div className="act-strip">
                            <Link className="act-btn act-btn-edit"
                              to={`/Category/update/${item._id}`} data-tip="Edit">
