...
const STATUS_STEPS = [
  { key: "Ordered",           label: "Ordered",          icon: "bi-clipboard-check" },
...
export default function AdminCheckoutShow() {
  ...
  useEffect(() => {
    ...
    if (list.length) {
      ...
      if (item) {
        setData({ ...item });
        setOrderStatus(item.orderStatus || "Ordered");
              ...
              <div className="panel">
                <div className="panel-header">
                  ...
                  <span className="badge text-bg-primary-subtle text-primary-emphasis ms-auto">
                    {data.books.length} item{data.books.length > 1 ? "s" : ""}
                  </span>
                </div>
                <div className="table-responsive">
                  <table className="table align-middle mb-0">
                    ...
                    <tbody>
                      {data.books.map((i, idx) => (
                        <tr key={i._id ?? idx}>
                          <td className="text-muted">{idx + 1}</td>
                          <td className="fw-semibold">{i.book?.name || "—"}</td>
                          <td>
